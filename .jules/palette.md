...
## 2026-06-29 - [Static Analysis Proximity & View Extraction]
**Learning:** For complex SwiftUI `Button` bodies (e.g., those with conditional loading states and `ProgressView`) that exceed 10 lines, extracting the label content into a `@ViewBuilder` property is the preferred pattern. This keeps the `Button` declaration compact, ensuring the `.hoverEffect()` modifier stays within the strict 10-line lookahead window used by `ai-analyzer.py` for visionOS readiness checks.
**Action:** Extract complex button labels into helper properties to maintain static analysis compatibility and code readability.
