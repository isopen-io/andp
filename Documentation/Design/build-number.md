...
> la logique max-build ou la logique timestamp ou la logique HEX commit."*

ANDP reads the build number (`CFBundleVersion`) from the IPA and uploads it —
today the *incrementing* is often delegated to an external tool. This feature makes ANDP
compute the next build number itself, so a release pipeline no longer needs
an external tool just for the build number. ANDP computes it; the build step applies it
...
```
andp build-number [bundle_id] --strategy <max-build|timestamp|commit> [options] [--json]
```
...
`--json` → `{"command":"build_number","ok":true,"strategy":"max-build",
...
### 2.1 `max-build` — ASC-aware, sequential (the max+1 logic)
...
- `--floor N` (default 0): the local project's current build number, so we never
  go *below* it even if ASC lags (a build still PROCESSING). It guarantees a
  strictly higher number than any build already known to Apple.
...
- `service.build_number(strategy, bundle_id=None, floor=0, fmt=None, sha=None,
  ...
  affects `max-build` (no creds → error, since a real ASC lookup is required).
...
`max-build` tested over a FakeSession build list.
...
11. max-build: floor=1254, asc_latest=1200 → "1255".
12. max-build: floor=0, asc_latest=1300 → "1301".
13. max-build: no builds, floor=0 → "1".
14. max-build: dry-run (no creds) → error envelope (needs ASC).
15. max-build: app not found → error.
...
**CLI**
20. `build-number me.app --strategy max-build --floor 1254` (FakeSession) → prints "1255", exit 0.
...
warning** (max-build strategy assumes integer builds); `--floor` remains the
...
marketing version** (the scales differ wildly: commit ~1e8, max-build ~1e3,
...
the account/managers; `max-build` reloads via `_managers_for` (like precheck) and,
...
`max-build` without `bundle_id` → exit **2**. Service-level failures (bad creds,
...
failure. Tests: `test_max_build_non_int_floor_is_envelope_not_crash`,
...
`(best, skipped)`; the max-build envelope carries a `warning` (and `source.skipped`)
...
`test_max_build_warns_when_dotted_versions_skipped`.
